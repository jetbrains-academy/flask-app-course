...
class TestClient(flask_unittest.ClientTestCase):
    ...
    def test_post_with_client(self, client):
        ...
        self.assertEqual((b'{"Posted a device":{"id":"100500","location":"somewhere","name":"TestDevice"'
    ...
    def test_delete_with_client(self, client):
        ...
        self.assertEqual(200, response.status_code, msg=f"DELETE request resulted in an unexpected response code {response.status_code}.")
        self.assertEqual(b'{"message":"Device deleted"}\n', response.data, msg="DELETE request resulted in unexpected response content.")
